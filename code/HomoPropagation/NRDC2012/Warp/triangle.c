...
typedef REAL1 **triangle;            /* Really:  typedef triangle *triangle   */
...
typedef REAL1 **subseg;                  /* Really:  typedef subseg *subseg   */
...
/* The vertex data structure.  Each vertex is actually an array of REAL1s.    */
/*   The number of REAL1s is unknown until runtime.  An integer boundary      */
/*   marker, and sometimes a pointer to a triangle, is appended after the    */
/*   REAL1s.                                                                  */

typedef REAL1 *vertex;
...
struct badtriang {
  ...
  REAL1 key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  REAL1 xkey, ykey;                              /* Coordinates of the event. */
...
REAL1 splitter;       /* Used to split REAL1 factors for exact multiplication. */
REAL1 epsilon;                             /* Floating-point machine epsilon. */
REAL1 resulterrbound;
REAL1 ccwerrboundA, ccwerrboundB, ccwerrboundC;
REAL1 iccerrboundA, iccerrboundB, iccerrboundC;
REAL1 o3derrboundA, o3derrboundB, o3derrboundC;
...
/* Other variables. */
  ...
  REAL1 xmin, xmax, ymin, ymax;                            /* x and y bounds. */
  REAL1 xminextreme;      /* Nonexistent x value used as a flag in sweepline. */
...
/* Read the instructions to find out the meaning of these switches.          */
  ...
  int nobisect;
  int steiner;
  REAL1 minangle, goodangle, offconstant;
  REAL1 maxarea;
...
#define elemattribute(otri, attnum)                                           \
  ((REAL1 *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
  ((REAL1 *) (otri).tri)[m->elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((REAL1 *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
  ((REAL1 *) (otri).tri)[m->areaboundindex] = value
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, REAL1 area)
...
REAL1 area;                                      /* The area of the triangle. */
#endif /* not ANSI_DECLARATORS */

{
  REAL1 dxoa, dxda, dxod;
  REAL1 dyoa, dyda, dyod;
  REAL1 oalen, dalen, odlen;
  REAL1 maxlen;
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->minangle = (REAL1) strtod(workstring, (char **) NULL);
	  } else {
            b->minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->maxarea = (REAL1) strtod(workstring, (char **) NULL);
            if (b->maxarea <= 0.0) {
...

{
  ...
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(REAL1) +
                        sizeof(int) - 1) /
                       sizeof(int);
...

{
  ...
  /*   where the index is measured in REAL1s.                           */
  m->elemattribindex = (trisize + sizeof(REAL1) - 1) / sizeof(REAL1);
  /* The index within each triangle at which the maximum area constraint  */
  /*   is found, where the index is measured in REAL1s.  Note that if the  */
  /*   `regionattrib' flag is set, an additional attribute will be added. */
  m->areaboundindex = m->elemattribindex + m->eextras + b->regionattrib;
  /* If triangle attributes or an area bound are needed, increase the number */
  /*   of bytes occupied by a triangle.                                      */
  if (b->vararea) {
    trisize = (m->areaboundindex + 1) * sizeof(REAL1);
  } else if (m->eextras + b->regionattrib > 0) {
    trisize = m->areaboundindex * sizeof(REAL1);
  }
...
#ifdef ANSI_DECLARATORS
int fast_expansion_sum_zeroelim(int elen, REAL1 *e, int flen, REAL1 *f, REAL1 *h)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int scale_expansion_zeroelim(int elen, REAL1 *e, REAL1 b, REAL1 *h)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  INEXACT REAL1 c;
  INEXACT REAL1 abig;
  REAL1 ahi, alo, bhi, blo;
  REAL1 err1, err2, err3;
...
#ifdef ANSI_DECLARATORS
REAL1 estimate(int elen, REAL1 *e)
#else /* not ANSI_DECLARATORS */
REAL1 estimate(elen, e)
int elen;
REAL1 *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
REAL1 counterclockwiseadapt(vertex pa, vertex pb, vertex pc, REAL1 detsum)
#else /* not ANSI_DECLARATORS */
REAL1 counterclockwiseadapt(pa, pb, pc, detsum)
vertex pa;
vertex pb;
vertex pc;
REAL1 detsum;
...
{
  INEXACT REAL1 acx, acy, bcx, bcy;
  REAL1 acxtail, acytail, bcxtail, bcytail;
  INEXACT REAL1 detleft, detright;
  REAL1 detlefttail, detrighttail;
  REAL1 det, errbound;
  REAL1 B[4], C1[8], C2[12], D[16];
  INEXACT REAL1 B3;
...
#ifdef ANSI_DECLARATORS
REAL1 counterclockwise(struct mesh *m, struct behavior *b,
                      vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
REAL1 counterclockwise(m, b, pa, pb, pc)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  REAL1 detleft, detright, det;
  REAL1 detsum, errbound;
...
#ifdef ANSI_DECLARATORS
REAL1 incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, REAL1 permanent)
#else /* not ANSI_DECLARATORS */
REAL1 incircleadapt(pa, pb, pc, pd, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...
{
  INEXACT REAL1 adx, bdx, cdx, ady, bdy, cdy;
  REAL1 det, errbound;

  INEXACT REAL1 bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  REAL1 bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  REAL1 bc[4], ca[4], ab[4];
  INEXACT REAL1 bc3, ca3, ab3;
  REAL1 axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  REAL1 bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  REAL1 cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  REAL1 abdet[64];
  int ablen;
  REAL1 fin1[1152], fin2[1152];
  REAL1 *finnow, *finother, *finswap;
  int finlength;

  REAL1 adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT REAL1 adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  REAL1 adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  REAL1 aa[4], bb[4], cc[4];
  INEXACT REAL1 aa3, bb3, cc3;
  INEXACT REAL1 ti1, tj1;
  REAL1 ti0, tj0;
  REAL1 u[4], v[4];
  INEXACT REAL1 u3, v3;
  REAL1 temp8[8], temp16a[16], temp16b[16], temp16c[16];
  REAL1 temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  REAL1 axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  REAL1 bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  REAL1 cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  REAL1 axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  REAL1 axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  REAL1 axtbctt[8], aytbctt[8], bxtcatt[8];
  REAL1 bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  REAL1 abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  REAL1 abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT REAL1 abtt3, bctt3, catt3;
  REAL1 negate;
...
#ifdef ANSI_DECLARATORS
REAL1 incircle(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
REAL1 incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  REAL1 adx, bdx, cdx, ady, bdy, cdy;
  REAL1 bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  REAL1 alift, blift, clift;
  REAL1 det;
  REAL1 permanent, errbound;
...
#ifdef ANSI_DECLARATORS
REAL1 orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   REAL1 aheight, REAL1 bheight, REAL1 cheight, REAL1 dheight,
                   REAL1 permanent)
#else /* not ANSI_DECLARATORS */
REAL1 orient3dadapt(pa, pb, pc, pd,
...
{
  INEXACT REAL1 adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  REAL1 det, errbound;
  ...
  REAL1 bc[4], ca[4], ab[4];
  INEXACT REAL1 bc3, ca3, ab3;
  REAL1 adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  REAL1 abdet[16];
  int ablen;
  REAL1 *finnow, *finother, *finswap;
  REAL1 fin1[192], fin2[192];
  int finlength;

  REAL1 adxtail, bdxtail, cdxtail;
  REAL1 adytail, bdytail, cdytail;
  REAL1 adheighttail, bdheighttail, cdheighttail;
  INEXACT REAL1 at_blarge, at_clarge;
  INEXACT REAL1 bt_clarge, bt_alarge;
  INEXACT REAL1 ct_alarge, ct_blarge;
  REAL1 at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
  INEXACT REAL1 bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
  INEXACT REAL1 adxt_cdy1, adxt_bdy1, bdxt_ady1;
  REAL1 bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  REAL1 adxt_cdy0, adxt_bdy0, bdxt_ady0;
  INEXACT REAL1 bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
  INEXACT REAL1 adyt_cdx1, adyt_bdx1, bdyt_adx1;
  REAL1 bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  REAL1 adyt_cdx0, adyt_bdx0, bdyt_adx0;
  REAL1 bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
  INEXACT REAL1 bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
  INEXACT REAL1 adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  REAL1 bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  REAL1 adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  REAL1 u[4], v[12], w[16];
  INEXACT REAL1 u3;
  int vlength, wlength;
  REAL1 negate;
  ...
  bdy = (REAL1) (pb[1] - pd[1]);
  cdy = (REAL1) (pc[1] - pd[1]);
  adheight = (REAL1) (aheight - dheight);
  bdheight = (REAL1) (bheight - dheight);
  cdheight = (REAL1) (cheight - dheight);
...
#ifdef ANSI_DECLARATORS
REAL1 orient3d(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd,
              REAL1 aheight, REAL1 bheight, REAL1 cheight, REAL1 dheight)
#else /* not ANSI_DECLARATORS */
REAL1 orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  REAL1 adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  REAL1 bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  REAL1 det;
  REAL1 permanent, errbound;
...
#ifdef ANSI_DECLARATORS
REAL1 nonregular(struct mesh *m, struct behavior *b,
                vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
REAL1 nonregular(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, REAL1 *xi, REAL1 *eta, int offcenter)
#else /* not ANSI_DECLARATORS */
...
{
  REAL1 xdo, ydo, xao, yao;
  REAL1 dodist, aodist, dadist;
  REAL1 denominator;
  REAL1 dx, dy, dxoff, dyoff;
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                   REAL1 minedge, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  vertex joinvertex;
  REAL1 dxod, dyod, dxda, dyda, dxao, dyao;
  REAL1 dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  REAL1 apexlen, orglen, destlen, minedge;
  REAL1 angle;
  REAL1 area;
  REAL1 dist1, dist2;
  subseg sptr;                      /* Temporary variable used by tspivot(). */
...
#ifdef ANSI_DECLARATORS
REAL1 circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, REAL1 ccwabc)
#else /* not ANSI_DECLARATORS */
REAL1 circletop(m, pa, pb, pc, ccwabc)
struct mesh *m;
vertex pa;
vertex pb;
vertex pc;
...
{
  REAL1 xac, yac, xbc, ybc, xab, yab;
  REAL1 aclen2, bclen2, ablen2;
...
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b,
                                  struct splaynode *splayroot,
                                  struct otri *newkey,
                                  vertex pa, vertex pb, vertex pc, REAL1 topy)
#else /* not ANSI_DECLARATORS */
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                REAL1 *triangleattriblist, REAL1 *trianglearealist,
...
struct behavior *b;
int *trianglelist;
REAL1 *triangleattriblist;
REAL1 *trianglearealist;
int elements;
int corners;
int attribs;
int *segmentlist;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < m->eextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        setelemattribute(triangleloop, j,
                         (REAL1) strtod(stringptr, &stringptr));
      }
#endif /* not TRILIBRARY */
    }
    ...
    if (b->vararea) {
      ...
      if (*stringptr == '\0') {
        ...
        area = (REAL1) strtod(stringptr, &stringptr);
      }
#endif /* not TRILIBRARY */
...
/*****************************************************************************/
/*                                                                           */
/*  scoutsegment()   Sstd::cout the first triangle on the path from one endpoint  */
...

{
  ...
  /* Sstd::cout the beginnings of a path from the first endpoint */
  /*   toward the second.                                   */
  ...
  /* Sstd::cout the beginnings of a path from the second endpoint */
  /*   toward the first.                                     */
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, REAL1 *holelist, int holes,
                REAL1 *regionlist, int regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  REAL1 segmentlength, nearestpoweroftwo;
  REAL1 split;
  REAL1 multiplier, divisor;
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    x = (REAL1) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    y = (REAL1) strtod(stringptr, &stringptr);
    vertexloop[0] = x;
    vertexloop[1] = y;
    ...
    for (j = 2; j < 2 + m->nextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        vertexloop[j] = (REAL1) strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
...
#ifdef ANSI_DECLARATORS
void transfernodes(struct mesh *m, struct behavior *b, REAL1 *pointlist,
                   REAL1 *pointattriblist, int *pointmarkerlist,
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, REAL1 **hlist, int *holes,
               REAL1 **rlist, int *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *holes = (int) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (REAL1 *) trimalloc(2 * *holes * (int) sizeof(REAL1));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i] = (REAL1) strtod(stringptr, &stringptr);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i + 1] = (REAL1) strtod(stringptr, &stringptr);
      }
    }
  } else {
    *hlist = (REAL1 *) NULL;
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    *regions = (int) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (REAL1 *) trimalloc(4 * *regions * (int) sizeof(REAL1));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
        ...
        if (*stringptr == '\0') {
          ...
          regionlist[index++] = (REAL1) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh *m, struct behavior *b, REAL1 **pointlist,
                REAL1 **pointattriblist, int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*pointlist == (REAL1 *) NULL) {
    *pointlist = (REAL1 *) trimalloc((int) (outvertices * 2 * sizeof(REAL1)));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (REAL1 *) NULL)) {
    *pointattriblist = (REAL1 *) trimalloc((int) (outvertices * m->nextras *
                                                 sizeof(REAL1)));
  }
...
void writeelements(struct mesh *m, struct behavior *b,
                   int **trianglelist, REAL1 **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if ((m->eextras > 0) && (*triangleattriblist == (REAL1 *) NULL)) {
    *triangleattriblist = (REAL1 *) trimalloc((int) (m->triangles.items *
                                                    m->eextras *
                                                    sizeof(REAL1)));
  }
  tlist = *trianglelist;
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               REAL1 *holelist, int holes, REAL1 *regionlist, int regions,
               int argc, char **argv)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void writevoronoi(struct mesh *m, struct behavior *b, REAL1 **vpointlist,
                  REAL1 **vpointattriblist, int **vpointmarkerlist,
                  int **vedgelist, int **vedgemarkerlist, REAL1 **vnormlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*vpointlist == (REAL1 *) NULL) {
    *vpointlist = (REAL1 *) trimalloc((int) (m->triangles.items * 2 *
                                            sizeof(REAL1)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (REAL1 *) NULL) {
    *vpointattriblist = (REAL1 *) trimalloc((int) (m->triangles.items *
                                                  m->nextras * sizeof(REAL1)));
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (REAL1 *) NULL) {
    *vnormlist = (REAL1 *) trimalloc((int) (m->edges * 2 * sizeof(REAL1)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...

{
  ...
  REAL1 cossquare;
  REAL1 triarea;
  REAL1 shortest, longest;
  REAL1 trilongest2;
  REAL1 smallestarea, biggestarea;
  REAL1 triminaltitude2;
  REAL1 minaltitude;
  REAL1 triaspect2;
  REAL1 worstaspect;
  REAL1 smallestangle, biggestangle;
  REAL1 radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (REAL1) (i + 1));
...

{
  struct mesh m;
  struct behavior b;
  REAL1 *holearray;                                        /* Array of holes. */
  REAL1 *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  if (b.poly || b.convex) {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      ...
      if (b.poly) {
        ...
        out->holelist = (REAL1 *) NULL;
        out->regionlist = (REAL1 *) NULL;
      }
#else /* not TRILIBRARY */
